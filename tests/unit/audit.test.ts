...
describe("audit engine", () => {
  ...
  it("detects multiline and parser HTML sinks and only trusts imported sanitizer libraries", async () => {
    await withTempProject(async (projectRoot) => {
      await writeMinimalProject(projectRoot, {
        architectures: ["secure-by-default"],
      });
      await writeFile(
        join(projectRoot, "src", "UnsafeHtml.tsx"),
        [
          'import parse from "html-react-parser";',
          "const props = { dangerouslySetInnerHTML: { __html: article.body } };",
          "function sanitize(value: string) { return value; }",
          ...
          "    }} />",
          "    <section {...props} />",
          "    <main dangerouslySetInnerHTML={{ __html: sanitize(article.body) }} />",
          "    {parse(article.body)}",
          "  </>",
          "}",
          "",
      ...
      await writeFile(
        join(projectRoot, "src", "SafeHtml.tsx"),
        [
          ...
          "    {parse(DOMPurify.sanitize(article.body))}",
          "  </>",
          "}",
          "",
        ].join("\n"),
      ...
      const result = await runAudit({ rootDir: projectRoot, mode: "all" });
      const unsafe = result.findings.filter((item) => item.rule === "NB-ARCH-011" && item.file === "src/UnsafeHtml.tsx");

      expect(unsafe).toHaveLength(4);
      expect(result.findings).not.toContainEqual(expect.objectContaining({ rule: "NB-ARCH-011", file: "src/SafeHtml.tsx" }));
    });
  });

  it("keeps manual runtime boundaries audited when an OpenAPI generator is installed", async () => {
    await withTempProject(async (projectRoot) => {
      await writeMinimalProject(projectRoot, {
      ...
      await writeFile(
        ...
        JSON.stringify(
          {
            private: true,
            dependencies: { react: "^19.0.0", vite: "^6.0.0", "react-router": "^7.0.0" },
            devDependencies: { orval: "^7.0.0", typescript: "^5.9.3" },
          },
          null,
          2,
        ),
      ...
      await mkdir(join(projectRoot, "src", "api", "generated"), { recursive: true });
      await writeFile(
        join(projectRoot, "src", "api", "manual.ts"),
        "export async function getManual() {\n  const response = await fetch('/manual');\n  const data = await response.json();\n  return data;\n}\n",
        "utf8",
      );
      await writeFile(
        join(projectRoot, "src", "api", "generated", "client.ts"),
        "// Generated by Orval\nexport async function getGenerated() {\n  const response = await fetch('/generated');\n  const data = await response.json();\n  return data;\n}\n",
        "utf8",
      );
      await writeFile(
        join(projectRoot, "src", "api", "mixed.ts"),
        [
          'import { parse } from "valibot";',
          "export async function getMixed() {",
          "  const firstResponse = await fetch('/validated');",
          "  const validatedRaw = await firstResponse.json();",
          "  const validated = parse(invoiceSchema, validatedRaw);",
          "  const secondResponse = await fetch('/unvalidated');",
          "  const unvalidated = await secondResponse.json();",
      ...
      expect(result.findings).toContainEqual(expect.objectContaining({ rule: "NB-ARCH-007", file: "src/api/manual.ts" }));
      expect(result.findings).not.toContainEqual(expect.objectContaining({ rule: "NB-ARCH-007", file: "src/api/generated/client.ts" }));
      expect(result.findings.filter((item) => item.rule === "NB-ARCH-007" && item.file === "src/api/mixed.ts")).toEqual([
        expect.objectContaining({ line: 7 }),
      ]);
    });
  });

  it("parses JSONC tsconfig files and does not treat awaiting params as remote data", async () => {
    await withTempProject(async (projectRoot) => {
      await writeMinimalProject(projectRoot, {
        architectures: ["modern-typescript", "error-loading-boundaries"],
      });
      await writeFile(
      ...
      expect(result.findings).not.toContainEqual(expect.objectContaining({ rule: "NB-ARCH-013" }));
